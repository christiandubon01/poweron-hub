...
## Shared Update â€” Team Cost Repair: Owner Classification Fix
...
- User-facing behavior changed:
  ...
  Helper text changes from 'Loaded = base Ã— 1.20x payroll multiplier' to 'Owner cost â€” no W-2 payroll burden'.
...
- Owner/W-2/1099 formulas (post-fix):
  ...
  W-2: loadedHourly = baseHourly Ã— payrollMult (unchanged)
  1099: loadedHourly = baseHourly (unchanged)
  Sentinel: id in ['me','owner','owner-virtual'] OR name === 'owner / me' â†’ 'owner'

- Team and Estimate integration notes:
  Team Cost Summary uses calcEmployeeCost â†’ calcMonthlyBreakdown â†’ getWorkerCostProfile â†’ resolveWorkerType. Fixed.
...
- Next agent should know:
  ...
  This is the display/calculation layer fix only â€” storage still lacks isOwner for stale records.

---

## Shared Update â€” Estimate Labor: Default Invalid Employee to Owner
...
- Root cause:
  getEmployeeDisplayName used `|| empId` as fallback â€” showed raw ID when employee not found.
  getRowEmployees had no validation â€” passed stale IDs straight to display functions.
  ownerInRoster check used raw e.isOwner===true, missing records identified only by name sentinel.

- Employee fallback behavior:
  - getEmployeeDisplayName: `|| empId` â†’ `|| 'Owner / Me'` (single line change)
