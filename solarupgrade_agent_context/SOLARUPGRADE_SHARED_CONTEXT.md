...
WHAT CHANGED:
- Used the custom Monthly Bill Comparison chart presentation as the render baseline: 575px SVG width, 170px SVG height, compact plot padding, full month labels, larger chart title/subtitle text, and readable legend styling.
- Aligned 24H Flow, 25 Yr Savings, Elec. Cost, and Cumulative chart canvases to the same compact 575x170 render feel.
- Kept the NEM 3.0 dark chart language: dark panels, subtle grid lines, restrained cyan/teal/yellow/emerald accents, and no aggressive glow.
- Increased chart subtab button click targets, font size, and padding for Monthly Bill, 24H Flow, 25 Yr Savings, Elec. Cost, Cumulative, and Payments while keeping the cyan/teal active state restrained.
- Kept Payments visually consistent with the larger title/subtitle treatment without changing payment assumptions or values.

WHAT WAS LEARNED:
- The prior post-height-polish charts were visually inconsistent because some charts used tall viewBoxes while Monthly Bill had already been tuned to a compact 575x170 canvas.
- The chart module can be normalized by presentation constants and typography only; no modeled data or estimate math needs to change.

LEARNED SKILLS / REUSABLE PATTERNS:
- Reuse a known-good chart canvas and padding profile across related SVG charts to preserve a consistent panel rhythm.
- Keep long axis labels readable by allowing small per-chart side-padding adjustments while preserving the shared viewBox size.

BUGS / RISKS:
- Screenshot QA is still recommended on desktop and narrow viewports to confirm the compact 575x170 charts and larger tab buttons feel right in the summary card.
- No formulas, values, NEM assumptions, persistence, Supabase behavior, or unrelated tabs were changed.
...
NEXT PHASE ADJUSTMENTS:
- Run screenshot QA on all six Solar Estimate summary chart subtabs, especially narrow widths, to confirm no horizontal overflow and that legends/axis labels remain readable.
...
WHAT CHANGED:
- Removed the green `Modeled install cost` card from Step 4 System Config.
- Removed the Step 4 `Settings cost` design summary row so Step 4 stays focused on system design controls.
- Kept Summary and Settings-driven cost calculation intact.
- Added typed `mainPanelUpgradeNeeded: boolean` to Solar Estimate data with safe default `false`.
- Added a clean premium dark `Main panel upgrade` toggle directly below the `Solar plus battery` toggle in Step 4.
- Updated the Solar Plus Battery toggle layout so label/helper text and the switch align cleanly without clipped knob positioning.
- Added `Main panel upgrade: Yes/No` to Step 4 design info and Step 5 Summary interview/system values.
- Added normalization so older saved estimates/drafts derive the main panel upgrade default from main breaker size when the saved boolean is missing.

WHAT WAS LEARNED:
- Main breaker IDs are stored as `100A`, `125A`, `150A`, `200A`, `225A`, `400A`, and `unknown`.
- Step 4 previously calculated the settings cost only for a local display card; Summary owns the actual settings-driven cost path.
- Existing saved estimates persist the whole `SolarEstimateData` object, so adding a typed boolean plus normalization is enough for restore compatibility.

LEARNED SKILLS / REUSABLE PATTERNS:
- Preserve explicit saved booleans during normalization, and only derive defaults for older records where the field is missing.
- Keep Step 4 design state distinct from Summary cost modeling by removing cost-only dependencies from the Step 4 component.

BUGS / RISKS:
- Screenshot QA should verify Step 4 at desktop and narrow widths, especially both toggle cards and battery-size conditional rendering.
- The toggle defaults from breaker size when Step 3 breaker changes; the user can still manually override it in Step 4 afterward.
- An unrelated local modification exists in `src/components/v15r/V15rSettingsPanel.tsx` and was intentionally left out of this scoped commit.
...
NEXT PHASE ADJUSTMENTS:
- Screenshot QA should confirm Step 4 no longer shows install cost and that Main Panel Upgrade defaults ON for `100A`, `125A`, `150A`, and OFF for `200A`, `225A`, `400A`, and `unknown`.
...
## GRAPH DASHBOARD AUDIT COMPLETION LOG
...
WHAT CHANGED:
- Audit-only pass of every graph rendered by Graph Dashboard.
- No application code changed because branch creation was declined.
- Context files updated with graph list, findings, risks, and next recommended fixes.

WHAT WAS LEARNED:
- `graph-dashboard` routes to `src/components/v15r/V15rDashboard.tsx`.
- Direct dashboard graph inputs mostly start from `getBackupData()` and apply `isActiveProject` / `isActiveServiceCall`.
- Revenue timeline query helpers call `getBackupData()` independently and pass raw `backup.projects` into services that filter only by status.
- `EVRChart` has the correct `projId || projectId` log matching guard, but `PvAChart` and `RCAChart` still use `projectId` only.
- CFOT chart data is event-sourced by week, while CFOT summary boxes are current snapshot calculations, so they can disagree.

LEARNED SKILLS / REUSABLE PATTERNS:
- Put archive/status filters at graph query boundaries, not after chart series are built.
- Normalize project log IDs once with `(log.projId || log.projectId)` before date grouping or cumulative series building.
- For dashboard graphs, keep summary cards and plotted series on the same aggregate when users are expected to compare them directly.

BUGS / RISKS:
- High: `PvAChart` actual series likely misses real field logs because it matches `log.projectId` only while backup logs use `projId`.
- High: `RCAChart` likely underreports costs/revenue for the same reason in both single-project and multi-project modes.
- High: Revenue timeline helpers can include archived projects in projections, overlaps, Gantt rows, and quote-vs-actual if archived projects still have `active`/`coming` status.
- Medium: `SixMonthForecastChart` reads raw `backup.projects` and filters by status only, so archived active/coming projects can leak into the forecast.
- Medium: `SCPChart` "Net Profit" subtracts material and mileage only; it does not subtract labor/op cost and may not read the canonical service-log material/mileage fields.
- Medium: RCA date ranges mix filtered costs with all-time `fin.paid` in multi-project mode.
...
NEXT PHASE ADJUSTMENTS:
- Recommended scoped fix branch: patch `PvAChart` and `RCAChart` to match `(log.projId || log.projectId)`, filter revenue timeline query project inputs through `isActiveProject`, filter `SixMonthForecastChart` through `isActiveProject`, and re-run manual graph QA.
- Consider a follow-up CFOT consistency pass to decide whether summary boxes should be snapshot KPIs or match the current plotted event-sourced right edge.

NEXT PHASE READY:
NO active build phase. Ready for a narrow Graph Dashboard fix pass.

COMPACT HANDOFF FOR NEXT CHAT:
Graph Dashboard audit complete, no commit. All 14 graph widgets in `V15rDashboard` were audited. Typecheck passes. No app code changed because branch creation was declined. Main findings: `PvAChart` and `RCAChart` use `log.projectId` only while real logs use `projId`; revenue timeline helpers and `SixMonthForecastChart` filter raw project statuses instead of using `isActiveProject`, allowing archived active/coming projects to leak; CFOT summary cards and plotted event-sourced lines can disagree; `SCPChart` likely overstates Net Profit by omitting labor/op cost.

---

## GRAPH DASHBOARD FIX PASS COMPLETION LOG
...
WHAT CHANGED:
- `PvAChart` and `RCAChart` now match field logs using `(log.projId || log.projectId)`.
- Revenue timeline read queries now filter projects through shared `isActiveProject` before graph transforms.
- `SixMonthForecastChart` now filters projects through shared `isActiveProject` before active/coming pipeline totals.

WHAT WAS LEARNED:
- Current field logs are keyed by `projId`; `projectId` is only a compatibility/fallback field in some paths.
...
BUGS / RISKS:
- Broader CFOT/SCP/RCA business-logic questions remain intentionally untouched.
- Localhost browser QA was not completed because sandboxed Vite startup could not read config and the escalated run was declined; user will audit manually.

NEXT PHASE READY:
Ready for user manual Graph Dashboard audit.
...
ROOT CAUSE:
Wave 7 overlay objects were recreated on renders and edge pulse speeds were overwritten each frame, causing the scene group/camera motion to appear to snap back periodically.

FIX:
Scene setup remains mount-only (`[]` deps). Overlay mode/data now sync through refs during render. Adapter caches overlays per mode. Animation uses accumulated root/star yaw and base edge speeds with overlay multipliers.
...
## Shared Update — Header Bar Audit (Cursor) — save button, SVC Unbilled, Coming Up quoted [2026-06-07]
...
FILES INSPECTED:
`V15rLayout.tsx`, `V15rSettingsPanel.tsx`, `backupDataService.ts`, `pipelineCalc.ts`, `V15rIncomeCalc.tsx`, `V15rEstimateTab.tsx`, `SixMonthForecastChart.tsx`, `AppShell.tsx`, `demoDataService.ts`
...
AUDIT SUMMARY:
- Global header is inline in `V15rLayout.tsx` (no separate HeaderBar component).
- Import/Export header icons call `exportBackup` / `importBackupFromFile`; Settings “Save Live Data Now” calls `forceSyncToCloud()`.
- Safest Save wiring: header Save button → `forceSyncToCloud()` with existing sync-status/toast/tenant guards.
- SVC Unbilled value from `getKPIs().svcUnbilled`; color hardcoded yellow in layout; change to red. Exposure non-compact pill incorrectly shows svcUnbilled (line 1610).
- “Coming Up Projects Total Quoted” has no header KPI; derive sum of `contract` for `resolveProjectBucket==='coming'` projects (not `pipelinePending` which mixes service data).
- Min implementation files: `V15rLayout.tsx` primary; optional `backupDataService.ts` if KPI added to `getKPIs()`.

RECOMMENDED CODEX ORDER:
1. SVC Unbilled red + Exposure display fix
2. Save button replaces Import/Export
3. Coming Up quoted KPI pill

INDEPENDENCE:
All three tasks are independent; Settings backup UI unchanged.

COMPACT HANDOFF FOR NEXT CHAT:
Header bar audit complete on `main`. Ready for Codex narrow fix pass in `V15rLayout.tsx`. Save → `forceSyncToCloud()`. SVC Unbilled yellow→red. Coming Up quoted = coming-bucket project `contract` sum. No code changes in this pass.

---

## Shared Update — Header Bar Fix Pass — Save button, red SVC Unbilled, Coming Up quoted KPI [2026-06-07]
...
ROOT CAUSE:
Global header still exposed backup Import/Export actions while Settings owned that workflow; SVC Unbilled was yellow; non-compact Exposure rendered `svcUnbilled`; and the header had no project-only Coming Up quoted KPI.

WHAT CHANGED:
- Header Import/Export controls, hidden file input, and layout-only import/export handlers were removed.
- Added a single Save button that calls `forceSyncToCloud()` with existing tenant/Supabase/sync-status/toast guard behavior.
- Added Coming Up KPI from active projects where `resolveProjectBucket(p) === 'coming'`, summing `contract` only.
- Changed SVC Unbilled to red.
- Fixed non-compact Exposure to show `safeKpis.exposure`.

MANUAL QA:
`npm.cmd run typecheck` passed. `git diff --check -- src/components/v15r/V15rLayout.tsx` passed. `http://localhost:5173` returned HTTP 200. Authenticated visual/browser QA was blocked because the in-app Browser runtime failed to start twice.

NEXT RECOMMENDED ACTION:
On authenticated localhost, verify the header has Save instead of Import/Export, Save triggers cloud-sync feedback, SVC Unbilled is red, Exposure displays the exposure value, Coming Up displays the correct coming-project contract total, and the header wraps cleanly at the current viewport.

COMPACT HANDOFF FOR NEXT CHAT:
Header fix pass complete on `main`. `V15rLayout.tsx` now has one guarded Save button wired to `forceSyncToCloud()`, red SVC Unbilled, corrected Exposure value, and a project-only Coming Up quoted KPI. Settings import/export unchanged. Typecheck passes; browser visual QA remains.

---

---

## Shared Update — Team Tab: Projection Scenarios and Overhead Recovery

- Agent: Claude Code Sonnet 4.6
- Branch: main
- Commit: (see below)
- Files changed: src/components/v15r/V15rTeamPanel.tsx
- Typecheck: PASS — zero errors

- User-facing behavior changed:
  Two new sections added to Team tab between Hypothetical Analysis and Employee Cards:
  1. Team Projection Scenarios: per-worker editable hrs/wk and wks/yr, cost/revenue/profit projections,
     scenario tabs (add/delete/rename), scenario totals strip, collapsible
  2. Overhead Recovery Bucket: fixed overhead from Employee Cost Structure × 12, editable billable
     hours target, overhead/hr calculation, per-worker contribution bars, team vs owner-only comparison

- Projection behavior:
  - Reads/saves scenarios in backup.settings.projectionScenarios
  - Uses getWorkerCostProfile for all cost calculations (owner/1099/W-2 rules applied correctly)
  - Owner badge from resolveWorkerType; sentinel-aware (name 'owner / me' detected)
  - Collapsible, scenario tabs, inline rename (double-click), add/delete scenarios

- Overhead recovery behavior:
  - Fixed overhead = settings.employeeCosts monthly total × 12
  - Billable hours target = settings.billableHoursYear (editable, persisted, default 1800)
  - Per-worker contribution bars using scenario projected hours
  - Team-vs-owner-only comparison callout (months faster)
  - Overhead shown as separate planning layer, not included in loaded cost

- Worker-cost formulas preserved:
  Owner loaded = base, 1099 loaded = base, W-2 loaded = base × payrollMult
  No changes to employeeCostUtils.ts or employeeTypes.ts

- Existing sections preserved:
  Org Pyramid, Employee Cost Structure, Charts, AI Cards, Owner KPI, Hypotheticals,
  Employee Cards, Team Cost Summary, Hours by Employee, Labor Chart, Per-Project Flow

- Risks / follow-up:
  - New employees not auto-added to existing scenarios (user must recreate or add manually)
  - billableHoursYear saves on every keystroke (no debounce)
  - Overhead bucket shows empty state if employeeCosts not configured
