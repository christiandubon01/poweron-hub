 ...
 *
 * Functions:
 *   analyzeCrewLog(log)       → Flag[] — detects anomalies in a single log
 *   reviewPendingLogs()       → { flagged, clean, summary } — batch review all unreviewed logs
...
export async function reviewPendingLogs(): Promise<ReviewResult> {
  ...
  // Fetch all unreviewed logs for this owner
  ...
  for (const log of (logs || [])) {
    ...
    if (detectedFlags.length > 0) {
      // Persist flags back to database
      await supabase
        .from('crew_field_logs')
...
/**
 * Fetches all logs from the last 24 hours, groups by crew member,
 * and returns a plain-English summary string.
 */
...
/**
 * Marks a single crew field log as reviewed by the owner.
 */
...
/**
 * Marks all flagged unreviewed logs for this owner as reviewed.
 */
