...
export const SCOPE_REGISTRY: Readonly<Record<DataScope, ScopeDescriptor>> = {
  ...
  'project.changeOrders': {
    ...
    strategy: 'id-merge',
    priority: 'critical',
    notes: 'ChangeOrder has createdAt/approvalAt but no updatedAt yet.',
  },
  'project.estimate': {
    scope: 'project.estimate',
    dataPath: 'projects[].matRows/mtoRows/laborRows/ohRows/estimateReference/phaseEstimateRows',
    owner: 'V15rEstimateTab / V15rMTOTab',
    level: 'nested',
    identityField: 'id (when rows have id)',
    needsTimestamp: true,
    needsTombstone: false,
    strategy: 'field-lww',
    priority: 'critical',
    notes: 'Rows are not all id-stamped; lastEstimateSyncAt is a project-level proxy timestamp.',
  },
  'project.payments': {
