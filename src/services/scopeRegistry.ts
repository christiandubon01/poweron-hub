...
export const SCOPE_REGISTRY: Readonly<Record<DataScope, ScopeDescriptor>> = {
  ...
  'project.estimate': {
    ...
    dataPath: 'projects[].laborRows/ohRows (Phase 6J rows only); scalars future',
    owner: 'V15rEstimateTab',
    level: 'nested',
    identityField: 'laborId/overheadId',
    timestampField: 'updatedAt',
    tombstoneField: 'deletedAt',
    needsTimestamp: false,
    needsTombstone: false,
    strategy: 'id-merge',
    priority: 'critical',
    notes: 'Phase 6J: project.estimate laborRows and ohRows only have delete-safe item-level nested merge via projectScopeMerge.ts. laborId/overheadId are internal stable identities; legacy id is preserved for UI/backward compatibility. Creates stamp createdAt/updatedAt, edits bump updatedAt, deletes write deletedAt/deletedBy tombstones, and live readers filter tombstones from UI/totals/context. Saves fetch latest remote and patch only projects[id].laborRows/ohRows through the existing remote-baseline path. Estimate scalars (contract, mileRT, miDays, laborPhaseColors, estimateReference, phaseEstimateRows, estimateVersions) remain future/unimplemented. project.materials is separate and already implemented under project.materials.',
  },
  'project.payments': {
