...
import { fetchPortalTrackerStateForLead } from '@/services/portal/portalService'
...
/** Read-only portal status badge shown in HUNTER during lead qualification. */
function PortalTrackingBadge({ lead }: { lead: HunterLead }) {
  ...
  useEffect(() => {
    if (!isPortalLead || !lead.id) return
    fetchPortalTrackerStateForLead(lead.id).then((state) => {
      if (!state?.request?.id) return
      setRequestId(state.request.id)
      setStatus(state.request.status ?? null)
    })
  }, [isPortalLead, lead.id])

  if (!isPortalLead || !requestId) return null

  return (
    <div className="flex items-center justify-between gap-3 p-2.5 bg-yellow-950/20 border border-yellow-700/25 rounded">
      <div className="flex items-center gap-2">
        <span className="text-xs font-semibold text-yellow-300">⚡ Portal Tracking</span>
        {status && (
          <span className="text-[10px] px-1.5 py-0.5 bg-yellow-900/40 text-yellow-400 border border-yellow-700/40 rounded font-mono capitalize">
            {status}
          </span>
        )}
      </div>
      <a
        href={`/portal/track/${requestId}`}
        target="_blank"
