...
function PortalStatusControls({ lead }: { lead: HunterLead }) {
  ...
  const isPortalLead = lead.source === 'customer_portal' || lead.sourceTag === 'customer_portal'

  useEffect(() => {
    let cancelled = false
    if (!isPortalLead || !lead.id) {
      setTracker(null)
      return
    }

    setLoading(true)
    fetchPortalTrackerStateForLead(lead.id)
      .then((state) => {
        if (cancelled) return
        setTracker(state)
  ...
  }, [isPortalLead, lead.id, lead.email])

  if (!isPortalLead || loading || !tracker?.request?.id) return null

  const doneTypes = new Set((tracker.timeline || []).map((event) => event.event_type))
  ...
  const sendReview = async () => {
    ...
    if (!result.success) {
      ...
      if (result.request) setTracker((prev) => prev ? { ...prev, request: result.request as any } : prev)
      return
    }
    ...
    <div className="space-y-3 p-3 bg-gray-800 rounded border border-yellow-700/30">
      ...
      <div className="flex flex-wrap gap-2">
        {PORTAL_LIFECYCLE_EVENT_TYPES.map((eventType) => {
          ...
          return (
            ...
            >
              {writing === eventType ? <Loader2 size={12} className="animate-spin" /> : isDone ? <CheckCircle size={12} /> : <Clock size={12} />}
      ...
      {reviewAlreadySent && (
        <div className="text-xs text-emerald-300">
          Review request already sent to {tracker.request.review_request_sent_to || tracker.request.email || 'customer'}.
        </div>
      )}
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Ask for honest feedback now that work is completed.</p>
              </div>
              <button onClick={() => setReviewModalOpen(false)} className="text-gray-500 hover:text-white">
                <X size={16} />
              </button>
            </div>
            ...
            {reviewAlreadySent ? (
              ...
              <div className="space-y-3">
                <label className="block">
                  <span className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Customer Email</span>
                  <input
                    type="email"
                    value={reviewEmail}
                ...
                <div className="rounded-lg bg-gray-900 border border-gray-800 p-3 text-xs text-gray-300 leading-relaxed">
                  The email thanks the customer and asks for honest feedback using the approved Google review link.
                </div>
                {reviewError && <div className="text-xs text-red-300">{reviewError}</div>}
                {reviewSentMessage && <div className="text-xs text-emerald-300">{reviewSentMessage}</div>}
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 mt-5">
              ...
              {!reviewAlreadySent && (
                ...
                >
                  {sendingReview ? <Loader2 size={13} className="animate-spin" /> : <Mail size={13} />}
                  Send Review Request
                </button>
              )}
            </div>
