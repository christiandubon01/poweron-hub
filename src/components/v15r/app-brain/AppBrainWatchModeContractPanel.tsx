...
import type { AppBrainRuntimeSnapshot, WatchSource } from './appBrainWatchModeContract'
...
const UTILITY_BADGES = [
  ...
  { label: 'HMR-safe watch', color: '#67e8f9' },

  { label: 'Generated snapshot', color: '#c4b5fd' },

  { label: 'Not live websocket', color: '#94a3b8' },
...
export default function AppBrainWatchModeContractPanel() {
  ...
  const snapshot = GENERATED_APP_BRAIN_RUNTIME_SNAPSHOT as AppBrainRuntimeSnapshot
  ...
  const skippedCount = snapshot.skippedNoMeaningfulChanges ?? 0
  const filesWritten = snapshot.filesWritten ?? []
  const filesSkipped = snapshot.filesSkipped ?? []
    ...
    >
      ...
      >
        ...
        <p className="text-xs text-blue-100/90 leading-relaxed">
          ...
          <span className="font-mono">npm run app-brain:watch</span>. Watch polls stable source inputs and skips

          timestamp-only rewrites to avoid Vite HMR scene resets. This panel shows the latest{' '}

          <span className="font-mono">generatedAppBrainRuntimeSnapshot.ts</span> file — generated snapshot only, not a

          websocket or live dashboard process.
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <StatCard label="Skipped no-op" value={skippedCount} color="#67e8f9" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        ...
        <div className="rounded-xl p-3" style={{ background: 'rgba(3,7,18,0.5)', border: '1px solid rgba(34,211,238,0.16)' }}>
          ...
          <p className="text-[10px] text-gray-500 mt-2">
            ...
            {snapshot.isWatchModeRunning ? 'yes' : 'no'} · HMR-safe watch:{' '}

            {snapshot.hmrSafeWatch ? 'yes' : 'no'} · Source changed:{' '}

            {snapshot.sourceChanged === false ? 'no' : 'yes'}
      ...
      {(filesWritten.length > 0 || filesSkipped.length > 0) ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          ...
          {filesWritten.length > 0 ? (
            ...
            <div className="rounded-xl p-3" style={{ background: 'rgba(3,7,18,0.5)', border: '1px solid rgba(52,211,153,0.16)' }}>
              ...
              <p className="text-[10px] uppercase tracking-widest text-emerald-200/80 mb-2">Files written</p>

              <div className="space-y-1 max-h-28 overflow-y-auto">

                {filesWritten.map((file: string) => (
          ...
          {filesSkipped.length > 0 ? (
            ...
            <div className="rounded-xl p-3" style={{ background: 'rgba(3,7,18,0.5)', border: '1px solid rgba(34,211,238,0.16)' }}>
              ...
              <p className="text-[10px] uppercase tracking-widest text-cyan-200/80 mb-2">Skipped (no meaningful change)</p>

              <div className="space-y-1 max-h-28 overflow-y-auto">

                {filesSkipped.map((file: string) => (
