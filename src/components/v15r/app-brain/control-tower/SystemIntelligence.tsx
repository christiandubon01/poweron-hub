import { ArrowLeft, Cpu, Gauge, ShieldCheck } from 'lucide-react'
import type { HostPresenceView } from '@/features/control-tower/controlTowerAdapter'
import { getAppBrainNode } from '../../appBrainMap'
import { CT_ROLES, ProviderModelBadge, TaskStatus, TowerPanel } from './ControlTowerPrimitives'
import { conciseTime, roleResult, sessionDuration, type TowerSession } from './sessionPresentation'
import { TEAM_ROLE_LABEL, teamNodeByRole, type AgentTeamTopology, type TeamRoleNode, type TeamRoleId } from './agentTeamTopology'
...
/** ATB-3: selected team-member detail — real provider/model/effort truth only (§6, §19). */
...
export default function SystemIntelligence({ run, presence, task, nodeId, team, selectedRoleId, onClear }: {
  run: TowerSession | null; presence?: HostPresenceView; task: PreviewTask | null; nodeId: string | null;
  team?: AgentTeamTopology | null; selectedRoleId?: TeamRoleId | null; onClear: () => void
}) {
  ...
  return <TowerPanel title={roleNode ? 'Team detail' : node ? 'Architecture detail' : task ? 'Task detail' : 'AI / System Intelligence'} className="ct-intelligence ct-inspector" action={<Cpu size={16} aria-hidden="true" />}>
    <div className="ct-intelligence-body">
      {(node || task || roleNode) && <button type="button" className="ct-intelligence-back" onClick={onClear}><ArrowLeft size={14} aria-hidden="true" />System Intelligence</button>}
      {roleNode && team ? <TeamRoleDetail roleNode={roleNode} team={team} run={run} /> : node ? <>
        ...
        <section className="ct-intelligence-section ct-ai-identity">
          <div className="ct-ai-symbol"><Cpu size={21} aria-hidden="true" /></div><div><span className="ct-eyebrow">{active ? 'Active AI' : 'Session routing'}</span><h3>{relevant?.role ?? 'Ready for work'}</h3><p>{active ? 'Executing reported task' : run ? 'No active execution' : 'No session selected'}</p></div>
        </section>
        ...
        {run && <details className="ct-disclosure"><summary>Scope &amp; candidate details</summary><p>{run.scope || run.objective}</p><p><ShieldCheck size={14} aria-hidden="true" /> {run.changeset === 'not-applied' ? 'Changes not applied. Candidate changes stay isolated.' : 'No candidate changes reported.'}</p>{run.candidatePaths?.map(path => <code key={path}>{path}</code>)}{run.verificationSummary && <p>{run.verificationSummary}</p>}</details>}
        <details className="ct-disclosure"><summary>Connection &amp; diagnostics</summary><dl className="ct-facts"><div><dt>Host</dt><dd>{presence?.state ?? 'Preview fixture'}</dd></div><div><dt>Available providers</dt><dd>{presence?.providers.join(', ') || 'Not reported'}</dd></div><div><dt>Heartbeat</dt><dd>{conciseTime(presence?.lastSeenAt)}</dd></div><div><dt>Session published</dt><dd>{conciseTime(run?.publishedAt)}</dd></div></dl><p>Usage refresh time unavailable. The Host does not publish usage telemetry.</p></details>
      </>}
    </div>
  </TowerPanel>
}