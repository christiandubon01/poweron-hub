...
const INNER_PROJ_LOGS_HIDE_GAPS_KEY = 'poweron:v15r:innerProjLogsHideGaps'
...
export default function V15rProjectLogsTab({ projectId, onUpdate, backup }: V15rProjectLogsTabProps) {
  ...
  const [showGaps, setShowGaps] = useState(() => {
    try { return localStorage.getItem(INNER_PROJ_LOGS_HIDE_GAPS_KEY) !== 'true' } catch { return true }
  })
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              const next = !showGaps
              setShowGaps(next)
              try { localStorage.setItem(INNER_PROJ_LOGS_HIDE_GAPS_KEY, String(!next)) } catch {}
            }}
      ...
      {sorted.length > 0 && (
        <div className="sticky top-0 z-10 bg-[var(--bg-input)] border border-gray-700 rounded-lg p-2 sm:p-3 mb-3 shadow-lg overflow-hidden">
        <div className="grid grid-cols-2 gap-2 text-left sm:grid-cols-3 lg:grid-cols-7 lg:text-center">
