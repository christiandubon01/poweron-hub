...
function V15rDashboardInner() {
  ...
  const cfotData = (() => {
    ...
    // Each CO is placed on the timeline by its own date (Correction pass JUN19):
    //   approvalAt → approvedAt → createdAt → date → project start.
    const coIsConfirmed = (co: any) => co.status === 'Approved' || co.status === 'Completed' || co.status === 'Invoiced' || co.status === 'Paid'
    const coIsApprovedUnpaid = (co: any) => co.status === 'Approved' || co.status === 'Invoiced' || co.status === 'Completed'
    const coEffectiveDate = (co: any, p: any): Date | null => {
      const raw = co.approvalAt || co.approvedAt || co.createdAt || co.date || p.startDate || p.plannedStart || p.start || p.createdAt
      if (!raw) return null
      const d = new Date(raw)
      return isNaN(d.getTime()) ? null : d
    }
    // Canonical paid parity (DASHBOARD-CFOT-MATH-FIX2-JUN19-2026-1): collectedAsOf sums
    // field-log payments only. The app's source-of-truth paid is getProjectFinancials().paid
    // = logged payments + manualPaidAdjustment (the p.paid scalar is deprecated/stale). Anchor
    // each project's as-of collected to that canonical total so Active Exposure at "Now"
    // matches the Money panel (fixes the ~$10k over-statement from missing collections).
    const farFuture = new Date(8640000000000000)
    const paidDeltaById = new Map<string, number>()
    for (const p of allProjects) {
      paidDeltaById.set(p.id, getProjectFinancials(p, backup).paid - collectedAsOf(p.id, farFuture))
    }
    const collectedCanonicalAsOf = (pid: string, asOf: Date) =>
      collectedAsOf(pid, asOf) + (paidDeltaById.get(pid) || 0)
    for (let i = 39; i >= 0; i--) {
      ...
      //                      subtraction — rises as projects/COs are added, not on collection.
      //   activeExp        = Active Exposure = base still owed (contract − canonical collected)
      //                      + approved-but-unpaid change orders. status === 'active' ONLY —
      //                      coming/future projects are NOT active exposure until they start.
      let projectsTotalExp = 0
      let activeExp = 0
      for (const p of allProjects) {
        ...
        for (const co of getLiveChangeOrders(p.changeOrders || [])) {
          ...
          if (coIsConfirmed(co)) confirmedCO += amt
          if (coIsApprovedUnpaid(co)) approvedUnpaidCO += amt
        }
        projectsTotalExp += contract + confirmedCO
        if (p.status === 'active') {
          const collected = collectedCanonicalAsOf(p.id, wEnd)
          activeExp += Math.max(0, contract - collected) + approvedUnpaidCO
        }
      }
      ...
      const totalExposure = projectsTotalExp                                    // Projects Total Exposure (projects only, gross)
      ...
      weeks.push({
        ...
        // Anchor the projected (Total Exposure) line at "Now" (i === 0) so the dashed
        // future series visually continues from the current Projects Total Exposure.
        projectedTotalExposure: i === 0 ? projectsTotalExp : null,
        isProjection: false,
      })
    }
    // ── Future projection (gray area after "Now") ──
    // DASHBOARD-FUTURE-EXPOSURE-JUN19-2026-1 (corrected to Total Exposure JUN19)
    // The dashed future line is Projected TOTAL Exposure — Projects Total Exposure carried
    // forward. Scheduled/upcoming projects (status 'coming' with a future plannedStart) are
    // excluded from the historical block by isProjectActiveAsOf; here we advance asOf into
    // each future week so a project's contract + confirmed COs join the projected total once
    // its scheduled start date passes. Active Exposure is intentionally NOT projected — it
    // only becomes real once a project is active. Future weeks never write historical fields.
    for (let i = 1; i <= 12; i++) {
      const wStart = new Date(todayWs)
      wStart.setDate(todayWs.getDate() + i * 7)
      const wEnd = new Date(wStart)
      ...
      for (const p of allProjects) {
        ...
        futureTotalExp += contract + confirmedCO
      }
      weeks.push({
        wk: 40 + i,
        svc: null,
        ...
        totalExposure: null,
        activeExposure: null,
        projectedTotalExposure: futureTotalExp,
        isProjection: true,
      })
    }
    return weeks
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[var(--bg-card)] rounded-lg border border-gray-700 p-6 lg:col-span-2" style={{ width: '100%', minWidth: 0 }}>
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-400 italic mt-1">Projects Total Exposure (contract + confirmed change orders) vs Active Exposure (active projects, approved unpaid) · Service Calls Exposure separate · Dashed = Projected Total Exposure (future projects) after Now</p>
            ...
            <div className="mt-4 grid grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
              <div className="bg-[var(--bg-input)] p-2 rounded">
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{background:'#dc2626'}}></div><span className="text-gray-500">Project Total Exp</span></div>
              ...
              <div className="bg-[var(--bg-input)] p-2 rounded">
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full" style={{background:'#fca5a5'}}></div><span className="text-gray-500">Service Calls Exp</span></div>
                <p className="font-bold font-mono text-red-300 mt-1">${cfotSummary.serviceExposure.toLocaleString()}</p>
              </div>
              <div className="bg-[var(--bg-input)] p-2 rounded">
