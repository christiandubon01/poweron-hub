...
function corsHeaders(req: Request): HeadersInit {
  ...
  const allowOrigin = configured.includes(origin) ? origin : configured[0] ?? 'http://localhost:5173'
...
serve(async (req: Request) => {
  ...
  if (!isUuid(body.assignment_id)) {
    return json(req, 400, { error: 'Valid assignment_id is required' })
  }
  ...
  if (assignmentError) {
    return json(req, 500, { error: 'Unable to load assignment' })
  }

  if (!assignment) {
    return json(req, 404, { error: 'Assignment not found' })
  }
  ...
  if (ownerAdminError) {
    return json(req, 500, { error: 'Unable to verify authorization' })
  }
  ...
  if (!authorized) {
    return json(req, 403, { error: 'Not authorized' })
  }

  const { data: attachments, error: attachmentError } = await serviceClient
  ...
  if (attachmentError) {
    return json(req, 500, { error: 'Unable to load snapshots' })
  }
  ...
  for (const attachment of orderedAttachments) {
    ...
    if (signError || !signed?.signedUrl) {
      return json(req, 502, { error: 'Unable to sign snapshot URL' })
    }
