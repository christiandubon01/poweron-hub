-- ══════════════════════════════════════════════════════════════════════════════
-- PowerOn Hub — Migration 104: Ownerless Assignment Fallback for Project-only
-- EMERG-PROJECT-ONLY-CLOCK-104
--
-- DEPENDS ON: 103 (assignment-backed Project eligibility in record_session_punch)
--
-- Root cause after migration 103:
--   Project-only mode raised 'Organization owner not configured' whenever
--   organizations.owner_id IS NULL — BEFORE Path A (app_state) or Path B
--   (eligible assignment). Live org has owner_id NULL, so Project-only Clock In
--   aborted even though eligible-assignment predicates all pass.
--   Assignment / Work Package Clock In never enters that guard and still works.
--
-- Fix:
--   CREATE OR REPLACE the canonical three-argument record_session_punch.
--   Project-only control flow:
--     1. Look up owner_id (no exception when null).
--     2. Path A — attempt app_state lookup ONLY when owner_id IS NOT NULL.
--     3. Path B — eligible-assignment fallback when Path A did not resolve.
--     4. Raise only when BOTH paths fail.
--   Path B does NOT require organizations.owner_id.
--
-- Preserved unchanged:
...
--   Duplicate clock_in 60s guard
--   sync_time_entry_from_sessions aggregation trigger
--   RLS policies
--   Migrations 097–103 SQL files (immutable)
--
...
BEGIN
  ...
  IF p_action = 'clock_in' THEN
    ...
    ELSE
      -- ── PROJECT-ONLY MODE ─────────────────────────────────────────────────────
      -- Path A: app_state Project when organizations.owner_id is present.
      -- Path B: eligible employee assignment for the exact canonical project_id
      --         when owner_id is null OR app_state does not resolve the Project.
      -- Path B does NOT require organizations.owner_id.
      ...
      -- Path A — only when an org owner exists to scope app_state
      ...
      ELSE
        ...
        -- Runs when owner_id is null OR app_state miss / unresolved Project.
        -- Does NOT attach the Work Package — session remains Project-only.
...
COMMENT ON FUNCTION public.record_session_punch(TEXT, UUID, TEXT) IS
  'Multi-session punch (migration 104). clock_in accepts either '
  'p_assignment_id UUID (assignment mode) or p_project_id TEXT (project-only: '
  'Path A app_state when owner_id present, else Path B eligible assignment; '
  'owner_id null does not block Path B; assignment_id remains NULL). Other '
  'actions resolve the active session server-side.';
...
-- 2. Early owner_id-null abort is gone; Path B is reachable without owner
-- SELECT pg_get_functiondef('public.record_session_punch(text,uuid,text)'::regprocedure)
--   LIKE '%Organization owner not configured%';  -- expected: false
-- SELECT pg_get_functiondef('public.record_session_punch(text,uuid,text)'::regprocedure)
--   LIKE '%IF v_owner_id IS NOT NULL THEN%';     -- expected: true
