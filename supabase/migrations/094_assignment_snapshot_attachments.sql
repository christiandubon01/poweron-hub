-- ============================================================================
-- PowerOn Hub - Migration 094: Assignment Snapshot Attachments
--
-- Version-scoped immutable Work Order snapshot attachments and atomic create RPC.
-- ============================================================================

BEGIN;

ALTER TABLE public.assignment_snapshots
  ADD COLUMN IF NOT EXISTS work_order_version INTEGER NOT NULL DEFAULT 1;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'assignment_snapshots_work_order_version_check'
      AND conrelid = 'public.assignment_snapshots'::regclass
  ) THEN
    ALTER TABLE public.assignment_snapshots
      ADD CONSTRAINT assignment_snapshots_work_order_version_check
      CHECK (work_order_version >= 1);
  END IF;
END;
$$;

ALTER TABLE public.assignment_snapshots
  DROP CONSTRAINT IF EXISTS assignment_snapshots_assignment_snapshot_key,
  DROP CONSTRAINT IF EXISTS assignment_snapshots_assignment_display_order_key;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'assignment_snapshots_assignment_version_snapshot_key'
      AND conrelid = 'public.assignment_snapshots'::regclass
  ) THEN
    ALTER TABLE public.assignment_snapshots
      ADD CONSTRAINT assignment_snapshots_assignment_version_snapshot_key
      UNIQUE (assignment_id, work_order_version, snapshot_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'assignment_snapshots_assignment_version_display_order_key'
      AND conrelid = 'public.assignment_snapshots'::regclass
  ) THEN
    ALTER TABLE public.assignment_snapshots
      ADD CONSTRAINT assignment_snapshots_assignment_version_display_order_key
      UNIQUE (assignment_id, work_order_version, display_order);
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'assignment_snapshots_display_order_max_check'
      AND conrelid = 'public.assignment_snapshots'::regclass
  ) THEN
    ALTER TABLE public.assignment_snapshots
      ADD CONSTRAINT assignment_snapshots_display_order_max_check
      CHECK (display_order >= 0 AND display_order <= 7);
  END IF;
END;
$$;

CREATE INDEX IF NOT EXISTS idx_assignment_snapshots_assignment_version_order
  ON public.assignment_snapshots (assignment_id, work_order_version, display_order, snapshot_id);
...
BEGIN
  ...
  IF v_count >= 8 THEN
    RAISE EXCEPTION 'A work order assignment version cannot have more than 8 snapshots';
  END IF;
...
COMMENT ON FUNCTION public.enforce_assignment_snapshot_limit() IS
  'Concurrency-safe guard that rejects a ninth snapshot attachment for a single assignment Work Order version.';
...
CREATE TRIGGER trg_assignment_snapshots_limit
  BEFORE INSERT OR UPDATE OF org_id, assignment_id, work_order_version ON public.assignment_snapshots
  FOR EACH ROW EXECUTE FUNCTION public.enforce_assignment_snapshot_limit();

DROP POLICY IF EXISTS assignment_snapshots_owner_admin_insert ON public.assignment_snapshots;
DROP POLICY IF EXISTS assignment_snapshots_owner_admin_update ON public.assignment_snapshots;
DROP POLICY IF EXISTS assignment_snapshots_owner_admin_delete ON public.assignment_snapshots;

DROP POLICY IF EXISTS assignment_snapshots_owner_admin_select ON public.assignment_snapshots;
...
REVOKE ALL ON public.assignment_snapshots FROM authenticated;
GRANT SELECT ON public.assignment_snapshots TO authenticated;

CREATE OR REPLACE FUNCTION public.prevent_attached_blueprint_snapshot_soft_delete()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL
    AND EXISTS (
      SELECT 1
      FROM public.assignment_snapshots ats
      WHERE ats.org_id = OLD.org_id
        AND ats.snapshot_id = OLD.id
      LIMIT 1
    )
  THEN
    RAISE EXCEPTION 'Snapshot is attached to an issued Work Order';
  END IF;
...
DROP TRIGGER IF EXISTS trg_blueprint_snapshots_prevent_attached_soft_delete
  ON public.blueprint_snapshots;
CREATE TRIGGER trg_blueprint_snapshots_prevent_attached_soft_delete
  BEFORE UPDATE OF deleted_at ON public.blueprint_snapshots
  FOR EACH ROW EXECUTE FUNCTION public.prevent_attached_blueprint_snapshot_soft_delete();
...
BEGIN
  ...
  IF cardinality(v_snapshot_ids) > 8 THEN
    RAISE EXCEPTION 'Maximum of 8 snapshots';
  END IF;
...
COMMENT ON FUNCTION public.create_employee_task_assignment_with_work_order_and_snapshots(
  ...
  'Atomically creates an assignment, immutable Work Order version 1, and ordered private snapshot attachments. Idempotent by org_id + client_request_id and rejects changed replay attachment lists.';

REVOKE ALL ON FUNCTION public.enforce_assignment_snapshot_limit() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.prevent_attached_blueprint_snapshot_soft_delete() FROM PUBLIC;
